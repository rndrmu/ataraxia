use crate::http::Http;
use crate::models::message::{to_value, CreateMessage, Message};
...
impl Context {
        ...
        F: FnOnce(&mut CreateMessage) -> &mut CreateMessage,
    {
        ...
        reqwest::Client::new()
            .post(format!("https://api.revolt.chat/channels/{}/messages", channel_id).as_str())
            .header("x-bot-token", self.token.clone())
            .header("content-type", "application/json")
    ...
    pub async fn join_voice_channel(&self, channel: &str) -> Result<(), serde_json::Error> {
        let res = reqwest::Client::new()
            .post(format!("https://api.revolt.chat/channels/{}/join_call", channel).as_str())
            .header("x-bot-token", self.token.clone())
            .header("content-type", "application/json")
            .send()
            .await
        ...
        // get result
        let json: Result<VoiceChannel, Error> =
            serde_json::from_str(res.text().await.unwrap().as_str());

        let vc = match json {
            Ok(json) => json,
            Err(e) => {
                error!("FAILED TO JOIN VOICE CALL -> {:?}", e);
                return Ok(());
            }
        };

        let mut voice_client =
            ataraxia_voice::vortex_socket::VoiceClient::new(vc.token, None).await;
    ...
    pub async fn kick_member(&self, server_id: &str, member_id: &str) {
        let _res = reqwest::Client::new()
            .delete(
                format!(
                    "https://api.revolt.chat/servers/{}/member/{}",
                    server_id, member_id
                )
                .as_str(),
            )
    ...
    pub async fn get_channel(&self, channel_id: &str) -> Result<RevoltChannel, serde_json::Error> {
        ...
        let json: Result<RevoltChannel, Error> =
            serde_json::from_str(res.text().await.unwrap().as_str());
