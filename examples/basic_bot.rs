...
impl EventHandler for Handler {
    ...
    async fn ready(&self, _ctx: Context, ready: Ready) {
        ...
        println!(
            "{:?} is connected!",
            ready.users.iter().map(|u| &u.username).collect::<Vec<_>>()
        );
    }
        ...
        if message.content == "!ping" {
                ...
                .send_message(&ctx.http, |r| {
                    r.content("hello!")
                        .set_masquerade(|masquerade| {
                            masquerade
                                .name("Rainer Winkler")
                                .avatar("https://i.imgflip.com/6bnywv.jpg")
                        })
                        .create_embed(|embed| {
                            embed
                                .title("Test Embed")
            ...
            match msg {
                Ok(msg) => {
                    let edited = msg
                        .edit(&ctx.http, |f| {
                            f.content("hello again!").create_embed(|f| {
                                f.title(":trol:").url("https://i.redd.it/ztfffav639991.jpg")
                            })
                        })
                        .await;
