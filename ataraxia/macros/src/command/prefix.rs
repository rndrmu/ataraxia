...
pub fn generate_prefix_action(inv: &Invocation) -> Result<proc_macro2::TokenStream, syn::Error> {
    ...
    Ok(quote::quote! {
        |ctx| Box::pin(async move {
            let ( #( #param_names, )* .. ) = ::ataraxia::parse_prefix_args!(
                ctx.serenity_context, ctx.msg, ctx.args, 0 =>
                #( #param_specs, )*
                #wildcard_arg
            ).await.map_err(|(error, input)| ataraxia::FrameworkError::ArgumentParse {
                error,
                input,
                ctx: ctx.into(),
            })?;
            ...
            inner(ctx.into(), #( #param_names, )* )
                .await
                .map_err(|error| ataraxia::FrameworkError::Command {
                    error,
                    ctx: ctx.into(),
                })
        })
